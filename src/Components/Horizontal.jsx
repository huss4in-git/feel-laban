...
function CardSlider() {
  ...
  return (
    <div className="w-full bg-[#faf9f6] py-6 sm:py-18">
      <div className="flex items-baseline gap-4 sm:gap-8 mb-0 sm:mb-8 overflow-x-auto whitespace-nowrap no-scrollbar justify-start sm:justify-center pl-6 sm:pl-0 mt-5 sm:mt-0">
      ...
      <div className="relative max-w-8xl mx-auto px-4 pt-6">
        {activeCategory !== "Shakes" && canScrollLeft && (
          ...
          >
            <svg width="60" height="30" viewBox="0 0 60 30" fill="none" stroke="#2563eb" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
        ...
        >
          {products.map((product, idx) => (
            <div key={`${activeCategory}-${idx}`} className="flex flex-col items-start min-w-[340px]" style={{ scrollSnapAlign: "start" }}>
              {/* FIXED IMAGE CONTAINER: Removed restrictive height/width, allowing big images */}
              <div className="bg-[#e8e6e3] p-10 flex flex-col items-center justify-center w-full min-h-[350px]">
                  <img src={product.image} alt={product.name} className="w-full h-auto max-h-[250px] object-contain drop-shadow-xl" />
              </div>

              <div className="text-[#2563eb] font-medium text-xl mt-4">{product.name}</div>
              <div className="flex gap-2 mt-2 flex-wrap">
                {product.tags.map((tag) => (
                  <span key={tag} className="border border-[#2563eb] rounded-full px-2 py-2 text-sm text-[#2563eb]">
                    {tag}
                  </span>
                ))}
              </div>
